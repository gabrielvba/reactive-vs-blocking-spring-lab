...
> O Dockerfile desta aplicação está na raiz do repositório (`app/consumers/ms-files-managment/Dockerfile`) e depende de arquivos do ecossistema inteiro. Execute os comandos a seguir a partir da raiz (`microservices-ecosystem/`).

```bash
# Build da imagem (contexto = raiz do repositório)
docker build -t ms-files-managment -f app/consumers/ms-files-managment/Dockerfile .
